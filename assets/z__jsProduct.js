...
class Product {
  ...
  enableSlideshow(selector, settings) {
    ...
    if (($sliderArrows || $thumbnails) && window.isScreenSizeLarge()) {
      ...
      $thumbnails.on('click', event => {
        ...
        if(window.innerWidth > 650){
          const isSlidesOff = document.querySelector('.product-template-shopibro .product-gallery');
          if(isSlidesOff) {
            const mainGallery = isSlidesOff.querySelector('.zoom-container');
            if(mainGallery){
              // mainGallery.click();
              // const fancybox_image = document.querySelector('.fancybox-image');
              // const clicked_image = event.currentTarget.querySelector('img');
              // if(fancybox_image && clicked_image){
              //   fancybox_image.src = clicked_image.src.replace('_300x', '_2048x');
              // }
            }
          }
        }
    ...
    $productGallery.on('settle.flickity', (event, index) => {
      const currentActive = document.querySelector('.pdt-active-slide');
      if(currentActive.classList){
        currentActive?.classList.remove('pdt-active-slide');
        document.querySelectorAll('.product-gallery__thumbnail')?.[index].classList.add('pdt-active-slide');
      }
